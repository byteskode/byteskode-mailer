...
MailSchema.statics.send = function(mail, options, done) {
    ...
    Mail._send(mail, options, function(error, _mail) {
...
MailSchema.statics.queue = function(mail, options, done) {
    ...
    Mail._send(mail, options, function(error, _mail) {
        ...
        if (error) {
            ...
            Mail.emit('mail:queued', _mail);

            //notify `mail:queued` kue worker if available
            //to process queued email
            if (Mail._queue) {
                var queueName = (_config.kue || {}).queue || 'mail:queued';
                Mail._queue.create(queueName, _mail.toObject()).save();
            }
    ...
    Mail.unsent(criteria, function(error, unsents) {
        ...
        if (error) {
            ...
            _.forEach(unsents, function(unsent) {
                ...
                if (Mail._queue) {
                    var queueName = (_config.kue || {}).queue || 'mail:queued';
                    Mail._queue.create(queueName, unsent.toObject()).save();
                }
